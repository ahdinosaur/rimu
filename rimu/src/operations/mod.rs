...
impl Operations {
    pub(crate) fn render(&self, engine: &Engine, env: &Environment) -> Result<Value, RenderError> {
        match self {
            Operations::Eval(op) => op.render(engine, env),
            Operations::Let(op) => op.render(engine, env),
            Operations::If(op) => op.render(engine, env),
        }
    }
}
