...
impl Error {
    ...
    pub fn write<C: ariadne::Cache<SourceId>>(self, cache: C, writer: impl Write) {
        ...
        let report = Report::build(ReportKind::Error, self.span.source(), self.span.start())
            .with_code(3)
            .with_message(msg)
            .with_label(
                Label::new(self.span)
